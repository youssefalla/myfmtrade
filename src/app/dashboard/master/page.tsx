...
import { LayoutDashboard, Eye, Users, TrendingUp, Banknote, Radio, Settings } from 'lucide-react'
    ...
    <div className="min-h-screen flex tf-page" style={{ fontFamily: SYS }}>
      ...
      <aside className="w-64 hidden md:flex flex-col p-6 gap-6 shrink-0 tf-sidebar">
        ...
        <div className="rounded-xl p-4 text-center" style={{ background: 'rgba(201,168,76,.08)', border: '1px solid rgba(201,168,76,.2)' }}>
          <div className="w-14 h-14 rounded-full grid place-items-center mx-auto mb-3 font-bold text-xl" style={{ background: 'linear-gradient(135deg,#E0C26A,#C9A84C)', color: '#1F2329', fontFamily: SYS }}>YA</div>
          <div className="text-sm font-semibold" style={{ color: 'var(--tf-text)' }}>Youssef Amrani</div>
          <div className="text-xs mt-0.5" style={{ color: '#C9A84C', fontFamily: SYS }}>Master Trader ✓</div>
          <div className="text-xs mt-2" style={{ color: 'var(--tf-subtle)' }}>Casablanca · 4y track</div>
        </div>
      ...
      <main className="flex-1 p-6 md:p-8 overflow-y-auto">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-6">
                ...
                {trades.map(({ pair, dir, result, pnl, time, copied }) => (
                  <div key={`${pair}-${time}`} className="flex items-center gap-3 py-2" style={{ borderBottom: '1px solid var(--tf-border)' }}>
                    ...
                    <div className="flex-1">
                      <div className="text-sm font-medium" style={{ color: 'var(--tf-text)', fontFamily: SYS }}>{pair} <span className="text-[10px]" style={{ color: 'var(--tf-subtle)' }}>· {copied} copied</span></div>
                      <div className="text-xs" style={{ color: 'var(--tf-subtle)' }}>{time}</div>
                    </div>
                    <div className="text-sm font-semibold" style={{ color: result === 'WIN' ? '#4ADE80' : '#F87171', fontFamily: SYS }}>{pnl}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="rounded-2xl p-6 tf-card-bg">
              <div className="flex items-center justify-between mb-5">
                <h2 className="text-base font-semibold" style={{ fontFamily: SYS, color: 'var(--tf-text)', letterSpacing: '-0.02em' }}>Top Followers</h2>
                <span className="text-xs" style={{ color: 'var(--tf-subtle)' }}>47 total</span>
              </div>
              <div className="space-y-3">
                {followers.map(({ init, name, city, since, profit }) => (
                  <div key={name} className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-full grid place-items-center font-bold text-xs shrink-0" style={{ background: 'rgba(201,168,76,.2)', color: '#C9A84C', fontFamily: SYS }}>{init}</div>
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-medium" style={{ color: 'var(--tf-text)' }}>{name}</div>
                      <div className="text-xs" style={{ color: 'var(--tf-subtle)' }}>{city} · {since}</div>
                    </div>
                    <div className="text-sm font-semibold shrink-0" style={{ color: '#4ADE80', fontFamily: SYS }}>{profit}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="rounded-2xl p-6 tf-card-bg">
            <div className="flex items-center justify-between mb-5">
              <div>
                <h2 className="text-base font-semibold" style={{ fontFamily: SYS, color: 'var(--tf-text)', letterSpacing: '-0.02em' }}>Your Marketplace Offers</h2>
            ...
            <div className="grid md:grid-cols-3 gap-4">
              {[
                ...
                <div key={title} className="rounded-xl p-4" style={{ background: 'var(--tf-card-inner)', border: '1px solid var(--tf-border)' }}>
                  ...
                  <div className="text-sm font-semibold mb-1" style={{ color: 'var(--tf-text)', fontFamily: SYS }}>{title}</div>
                  <div style={{ fontFamily: SYS, color: '#C9A84C', fontWeight: 600 }}>{price}</div>
                  <div className="text-xs mt-2" style={{ color: 'var(--tf-subtle)' }}>{sales} sales</div>
                </div>
              ))}
            </div>
          </div>
