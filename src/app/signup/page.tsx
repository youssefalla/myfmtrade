    ...
    <div className="min-h-screen flex items-center justify-center px-4 tf-page">
      <div className="w-full max-w-lg">
        ...
        <div className="rounded-2xl p-8 tf-card-gold">
          ...
          <p className="mt-2 text-sm" style={{ color: 'var(--tf-muted)' }}>Join Morocco&apos;s top copy-trading platform.</p>
          ...
          <div className="mt-6 grid grid-cols-2 gap-3">
            {(['trader', 'master'] as const).map(r => (
              <button key={r} type="button" onClick={() => setRole(r)}
                ...
                <div className="text-lg mb-1">{r === 'master' ? '👑' : '📈'}</div>
                <div className="text-sm font-semibold" style={{ color: 'var(--tf-text)' }}>
                  {r === 'master' ? 'Master Trader' : 'Copy Trader'}
                </div>
                <div className="text-xs mt-1" style={{ color: 'var(--tf-subtle)' }}>
